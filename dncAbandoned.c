  ...
  */
  ...
  while (1) {
    ...
    */
    ...
    gain = (25.0f/32.0f)*gain + (1.0f/32.0f)*prev_gain[0] + (2.0f/32.0f)*prev_gain[1] + (3.0f/32.0f)*prev_gain[2];
   

    //apply the gain
    for(int i = 0; i < capture_frames; i++){
     // if(inputSamples[0][i] > 0.10)
        inputSamples[0][i] *= gain;
      //if(inputSamples[1][i] > 0.10)
    ...
    for(int i = 0; i < capture_frames; i++){
      ...
      small = (char)(tempSum%256);
      big = (char)(tempSum/256);
